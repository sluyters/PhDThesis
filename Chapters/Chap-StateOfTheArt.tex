...
Other publications have focused on using some properties of input data to trigger segmentation, such as speed and acceleration. For instance, in~\cite{Chen:2016}, Chen \etal used the energy and root mean square (RMS) of raw EMG signals to properly segment gestures (energy-based segmentation). 
Finally, techniques such as Machete~\cite{Taranta:2021} rely on simple recognizers that continuously evaluate input data to determine the possible boundaries of gestures.

\textbf{TODO - Discuss DTW for segmentation and link to dynamic recognition} see https://doi.org/10.1016/j.patcog.2018.02.011 and https://doi.org/10.1016/j.patrec.2013.09.009 
...
Arm fatigue poses a significant challenge for mid-air gesture interaction, as badly designed gesture sets can reduce the time users can interact with a system before needing to rest~\cite{Hansberger:2017}. This phenomenon, often dubbed the ``gorilla arm effect'', has led to various metrics being proposed to quantify user arm fatigue. 
...
%
Jang \etal~\cite{Jang:2017} noted that the CE metric has several limitations: it does not take into account rest and its effect on cumulative fatigue, makes general assumptions about users' shoulder strength, and does not apply well to interaction with low exertion levels. 
They proposed the three-compartment muscle (TCM) model, which addresses these challenges to accurately quantify cumulative muscle fatigue. \textbf{TODO}
% Jang \etal expanded on this work

Hansberger \etal~\cite{Hansberger:2017} investigated a set of gestures that produce less discomfort and fatigue. Compared with keyboard and found similar results, with participants reporting low levels of exertion in both cases and being able to complete a 30 minutes game session, while badly designed gesture set max 12 minutes.
They suggested that the type of sensor could impact fatigue. For instance, some vision-based sensors like the LMC require users' hands to be within the field of view of the sensor, which may result in less ideal hand position, and thus more fatigue. IMUs, while having other limitations, don't place constraints on the position of the hand. \textbf{TODO}
%
Siddhpuria \etal~\cite{Siddhpuria:2017} \textbf{TODO}
%
Jakobsen \etal~\cite{Jakobsen:2015} \textbf{TODO}


\subsubsection{Affordance and Discoverability} \label{sec:state_of_the_art:overview:challenges:discoverability}
\textbf{TODO}
``Affordances reflect the possible relationships among actors and objects''~\cite{Norman:1999}
The concept of perceived affordance~\cite{Norman:1988,Norman:1999} of gesture-based interfaces (+ signifiers are ``the perceivable part
of an affordance''~\cite{Norman:2008}, \ie deliberate or incidental clues about how an object is supposed to be used.), challenges for gestural UI, then implication for discoverability


Physical constraints: how to communicate physical constraints of the system when they are not as straightforward to grasp as, \eg the limited dimensions of a screen? For instance, how to communicate limited field of view of the system? How to communicate if a system requires the user to be positioned precisely wrt the sensor?

Logical constraints

Cultural constraints, learned conventions. For instance, using a swipe gesture makes sense for many people that used touchscreens before, as they have adopted this convention for 2D gestures. Leveraging these existing conventions may lower the entry barrier, but should be cautious, as not everyone shares the same conventions (\eg, like with a thumb or like by drawing a check mark). But also avoid to violate existing ones, as to not create more confusion. GESs can help.

Add signifiers, clues for what is possible and what is not
Social signifiers: could show a new user the actions from a previous user, or encourage new users to watch other users interact

Feedback to understand the result of our action

Especially crucial in public displays, where need to grasp user attention directly~\cite{Walter:2013}

Many challenges for gesture explanation: simple to understand, avoid distracting users that already know how to interact, integrate nicely in the interface~\cite{Rovelo:2015}.

In~\cite{Walter:2013}, Walter \etal investigated three strategies to reveal an initial gesture on interactive public displays: spatial division (an explanation of the gesture is displayed on part of the screen), temporal division (the application is interrupted to reveal the gesture in full screen), and integration (visual cues are integrated in the application). They showed that spatial division worked well. Temporal division triggered users to leave while the cue was shown, as it interrupted their interaction with the application.

% Possible solutions
Octopocus~\cite{Bau:2008} and \cite{Kristensson:2011} 2D gestures, suggestion of continuation of trajectory

%
Gestu-Wan~\cite{Rovelo:2015} applied the principles from Octopocus to mid-air gesture interaction. They compared their implementation with a printed guide of gestures and observed \textbf{TODO}
%
ShapelineGuide~\cite{Alt:2018} 3D
%
White \etal~\cite{White:2007} 3D, compare different methods for showing how to interact with objects in AR (text, diagram, and ghosting)

\textbf{TODO - Discuss how to communicate system limitations}



% \subsubsection{Communicating User Errors} \label{sec:state_of_the_art:overview:challenges:user-errors}
% \textbf{TODO}
