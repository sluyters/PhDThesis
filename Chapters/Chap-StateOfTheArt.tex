...
\subsubsection{User Fatigue} \label{sec:state_of_the_art:overview:challenges:gorilla-arm}
\textbf{TODO}
Gorilla arm effect
\cite{Hincapie:2014} arm fatigue % say how the compute the metric
\cite{Jang:2017} cumulative arm fatigue


\cite{Hansberger:2017} users tired after about 12 minutes on average with classical mid-air gestures, \ie swipe left, swipe right, and extend arm, each performed with the hand at the level of the shoulder. 


Hansberger \etal~\cite{Hansberger:2017} investigated a set of gestures that produce less discomfort and fatigue. Compared with keyboard and found similar results, with participants reporting low levels of exertion in both cases and being able to complete a 30 minutes game session.
Suggested that the type of sensor could impact fatigue. For instance, some vision-based sensors like the LMC require users' hands to be within the field of view of the sensor, which may result in less ideal hand position, and thus more fatigue. IMUs, while having other limitations, don't place constraints on the position of the hand. 


\subsubsection{Gesture Discoverability and Affordance} \label{sec:state_of_the_art:overview:challenges:discoverability}
\textbf{TODO}
...
%
Gestu-Wan~\cite{Rovelo:2015} applies the principles from Octopocus to mid-air gesture interaction. Compared with a printed guide of gestures
%
...
We undertook a Systematic Literature Review (SLR)~\cite{Kitchenham:2010} to determine the algorithms used for LMC-based gesture recognition (\fig~\ref{fig:state_of_the_art:lmc:prisma}). The query \texttt{RQ=``leap motion'' AND ``gesture recognition''} was run in the ACM Digital Library\footnote{See \url{https://dl.acm.org}} and resulted in 390 publications, from which $N{=}43$ were identified as relevant. \textbf{TODO} Compared to our SLR on radar-based gesture recognition (Section~\ref{sec:state_of_the_art:radar}), this SLR was more secondary and we thus limited its scope to a single relevant library. The ACM Digital Library was identified as a good option for computer science SLR~\cite{Gusenbauer:2020}. Its large database should provide a good overview of the literature on LMC-based gesture recognition while minimizing our effort.
