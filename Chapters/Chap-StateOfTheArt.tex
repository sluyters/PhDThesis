...
\textbf{TODO}
Arm fatigue in mid-air interaction, also known as ``gorilla arm effect'', can be a real problem for mid-air gesture interaction, as badly designed gesture sets can greatly reduce the time during which users are able to interact with a system before getting too tired~\cite{Hansberger:2017}. 
This problem has been studied in multiple papers and different metrics have been proposed to quantify user arm fatigue. For instance, Hincapie \etal~\cite{Hincapie:2014} proposed a metric to \textbf{TODO}. Jang \etal studied cumulative arm fatigue.
...
\textbf{TODO}
The affordance of gesture-based interfaces (+ signifiers are ``the perceivable part
of an affordance''~\cite{Norman:2008}, deliberate or incidental clues about how an object is supposed to be used. - Donald Norman), challenges for gestural UI, then implication for discoverability
