...
Even a good gesture set might not satisfy every user, due to different preferences, disabilities, etc.. Allowing users to customize gestures can result in higher memorability and usability over pre-designed gestures sets~\cite{Oh:2013,Nacenta:2013} and result in more inclusivity for users that suffer from some sort of disability. Beside changing the gesture set, customization enables users to provide their own gesture samples to improve the accuracy of the system for them in particular.
% 
However, allowing customization comes with multiple challenges.

First, the gesture recognition pipeline must support custom gestures.
Some techniques for gesture recognition, template matching algorithms like \$P+~\cite{Vatavu:2017a}, Jackknife~\cite{Taranta:2017}, or 3 cent~\cite{Caputo:2017}, intrinsically support customization ``in real time''. Deep learning can also support customization quickly without complete retraining, but issues may arise and should be alleviated, such as catastrophic forgetting, where neural networks ``forget'' their previously acquired knowledge when trained with new classes~\cite{Schak:2019}.

Then, care should be given to design customization UIs that are usable by everyone, in particular, people with a disability who may not be happy with the pre-designed gestures. This UI should ideally integrate seamlessly into a gesture-based application.
%
...
%
Xu \etal~\cite{Xu:2022} looked into implementing gesture recognition with a smartwatch in a way that allows users to record new gestures. The tool checks if gestures are not too similar to existing gestures or to daily activities (\eg cooking or walking), if the samples produced are consistent, and if the accuracy is not too low, and provides feedback accordingly. While it allows users to name the gestures, it does not provide a visual representation of the samples performed by the user, bringing us to our last point.

How to represent gestures to users? While this is rather trivial with 2D gestures, this gets more complicated with 3d gestures. For instance, while vision-based sensors can record videos, or extract skeleton data from the images, the data provided by other sensors, like radars or IMUs, may be more difficult to visualize.\textbf{TODO}
