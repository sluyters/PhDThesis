...
\begin{itemize}
    \item \custominlinehighlight{Recognizer}: a nominal variable with two conditions, representing the static recognizers tested (Appendix~\ref{app:quantumleap-modules:static-recognizers}): GPSDa, a custom recognizer with  $\alpha{=}0.65$) and \$P\textsuperscript{3}+, a implementation of Vatavu's \$P+~\cite{Vatavu:2017a} that supports 3D gestures.
    \item \custominlinehighlight{Number of Articulations}: a numerical variable with two conditions, representing the number of articulations used by the recognizers: $A{\in}\{6,16\}$.
    \item \custominlinehighlight{Number of Templates}: a numerical variable with eight conditions, representing the number of templates used to train the recognizers: $\splitatcommas{T{\in}\{1,2,4,8,16,32,64,128\}}$.
\end{itemize}
...
\begin{enumerate}
    \item \custominlinehighlight{Recognizer}: a nominal variable with five conditions, representing the dynamic recognizers tested:  3\textcent~\cite{Caputo:2017}, \$P\textsuperscript{3} (adapted from~\cite{Vatavu:2012}), \$P\textsuperscript{3}+ (adapted from~\cite{Vatavu:2017a}), \$Q\textsuperscript{3} (adapted from~\cite{Vatavu:2018}), and Jackknife~\cite{Taranta:2017}. The recognizers are detailed in Appendix~\ref{app:quantumleap-modules:dynamic-recognizers}.
    \item \custominlinehighlight{Number of Articulations}: a numerical variable with three conditions, representing the number of articulations taken into account by the recognizers: $A{\in}\{1, 6, 16\}$.
    \item \custominlinehighlight{Number of Templates}: a numerical variable with four conditions, representing the number of templates used to train the recognizers: $\splitatcommas{T{\in}\{1,2,4,8\}}$.
\end{enumerate}
...
\begin{table*}[t]
    ...
    \end{tabular}
    % \vspace{-8pt}
    \caption{The 17 atomic tasks, potential target content, and associated gestures.}
    % \vspace{-10pt}
...
The KPI (Key Performance Indicator) computed by UEQ+~\cite{Hinderks:2019} started from 1.44 for session 1 to 1.74 for session 2, which confirms the overall positive evolution.
...
\begin{figure}[!t]
    ...
    \begin{subfigure}{.8\textwidth}
        \centering
        \fbox{\includegraphics[width=.96\linewidth]{Figures/LUI/Customization/recorder.pdf}} 
        \captionsetup{width=.99\linewidth}
...
Other limitations of \lui are related to our development method, specifically, the design of the gesture set.
%
First, user fatigue may become a problem during long interactions with \lui, as mid-air gestures can be more tiring than traditional interaction methods~\cite{Siddhpuria:2017, Jakobsen:2015}. 
%
In addition, while user-elicited gesture sets have been shown to improve the immediate usability of an application~\cite{Wobbrock:2005}, the GES procedure followed in this chapter is not without flaws.
%
For instance, our choice of illustrations of the referents may have influenced participants' gesture propositions. 
The small sample size of our study may also not accurately represent the majority of the population, as gesture propositions are affected by participants' cultural background~\cite{Archer:1997}. 
Finally, GESs usually rely on subjective criteria for gesture clustering, which may further impact their findings~\cite{Vatavu:2019}. 
...
The \lui application inherits multiple ISO/IEC 25010 software quality properties~\cite{iso25010} from its use of \ql and its testing tool (Sections~\ref{chap:quantumleap} and~\ref{chap:quantumleap-testing}), including \textit{maintainability} and \textit{portability}.
%
In addition, our methodology supports the \textit{usability} property, by encouraging developers to involve end users in the development process of their application, by conducting GESs to identify user preferences, and by evaluating their application among potential users.
